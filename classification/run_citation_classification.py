...
from classification.citances_processor import citances_output_modes as output_modes
from classification.citances_processor import citances_convert_examples_to_features as convert_examples_to_features
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.classification_type == "multiclass":
                ...
                loss = loss_func(logits, batch[3].type_as(logits))
...
def evaluate(args, model, tokenizer, prefix="", processor=None):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                if args.classification_type == "multiclass":
                    ...
                    logits = outputs[0]
                    loss_func = BCEWithLogitsLoss()
                    tmp_eval_loss = loss_func(logits, batch[3].type_as(logits))
                    #tmp_eval_loss = loss_func(logits.view(-1, num_labels), batch[3].type_as(logits).view(-1, num_labels))  # co

                eval_loss += tmp_eval_loss.mean().item()
            nb_eval_steps += 1
            if preds is None:
                preds = logits.detach().cpu().numpy()
                out_label_ids = inputs["labels"].detach().cpu().numpy() if "labels" in inputs else batch[3].detach().cpu().numpy()
            else:
                out = inputs["labels"].detach().cpu().numpy() if "labels" in inputs else batch[3].detach().cpu().numpy()
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                out_label_ids = np.append(out_label_ids, out, axis=0)
        ...
        if args.do_eval and not args.do_test:
            ...
            output_predictions_file = os.path.join(eval_output_dir, prefix, "predictions.txt")
            output_predictions_probs_file = os.path.join(eval_output_dir, prefix, "predictions_probs.txt")
